...
const BookingV2 = () => {
  ...
  useEffect(() => {
    ...
    const checkin = searchParamsHook[0].get('checkin');
    const checkout = searchParamsHook[0].get('checkout');
    const adults = searchParamsHook[0].get('adults');

    if (checkin && checkout && adults) {
      const params = { checkin, checkout, adults: parseInt(adults) };
  ...
  const handleSearch = async (params: SearchParams) => {
    ...
    try {
      ...
      // Mostrar todos os quartos retornados pela API, independentemente do valor
      const pricedAndImagedResults = mergedResults;
